...
# - convert timestamp to datetime: column type timestamp will break after 19.1.2038
    ...
    foreach($lines as $line){
        ...
        switch($token[0]){
            ...
            case 'datetime':
                ...
                break;
            case 'timestamp':
                $args[1] = 'datetime' . substr($args[1], strlen($token[0]));
                $str=injectDefault($args,$tableName, "'0000-00-00 00:00:00'", $lineNr, $args[1]);
                if ($str === '') {
                    # Default already present — emit ALTER only to change the column type.
                    $dirty = true;
                    $str = "ALTER TABLE $tableName CHANGE " . $args[0] . " " . implode(' ', $args);
                }
                break;
            case 'time':
